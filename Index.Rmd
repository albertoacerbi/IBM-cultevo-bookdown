...
Models can also help to understand the consequences of our theories. Social systems, as many others, are typically under the influence of different, co-existing, forces. While the effects of these forces, taken singularly, can be easy to predict, they interact with each others in ways that are non-obvious. This is the basic idea behind defining these systems as 'complex' systems. With verbal descriptions, figuring out the effects of interactions is left to our insights. With models, we just let them run, including in our programs (some of) these forces and we can just see what happens.

## Why individual-based models?

'Model' is itself a quite vague term. As we just saw, verbal descriptions can be considered models; a map of a building can be considered a model of that building, indeed a useful one. In this book, we mostly refer to formal models that can ultimately be described in mathematical terms. 

There are some distinctions that can be made among formal models. First, some models described the behaviour of a system at population-level. A model can specify, say, that the frequency of a cultural trait $A$ at time $t$ depends on its frequency at time $t-1$, perhaps it doubles at each time step. We can write a simple equation, and we can calculate how the frequency of this cultural trait grows in time. Other models, instead, describe the behaviour of a system at individual-level. Imagine the same question, but now we specify that, in a population of $N$ individuals, each individual, say, look at two other individuals and, if at least one of them have the trait $A$, it copies it. Again, we can write down some equations and calculate how the frequency of $A$ grows.    

Another distinction concerns models that are analytically tractable and models that are not. The former are mathematical models that consist of sets of equations that can be solved to find specific answers (e.g. equilibria points). Our population-level model described above would fit this description. A big advantage of these models is that they can provide insight into the dynamics of a system for a wide range of parameters, or exact results for specific questions. However, this approach requires the studied dynamics to be rather simple. It would be more difficult (or perhaps impossible) to write and analytically solve the systems of equations necessary to describe our second model. Often, when we want or need to describe the behaviour at individual level - if, for example, individuals differ in their characteristics, exhibit learning or adaptation, or are embedded in social networks - we cannot solve equations and we need to write down a code that describes some rules by which those individuals interact or change over time. These are individual-based models (IBMs). These models are both individual-level (i.e. they describe the behaviour of a system at the level of individuals) and simulations (i.e. they consists of descriptions of individuals and rules and simulate what happens to them). 

IBMs have a greater flexibility than mathematical models. Due to their structure they are also often more intuitive to understand, especially for people with little training in mathematics. However, it is also important to be aware of the downsides of IBMs. For example, generalisations are often not possible and statements only hold for parameters (or sets thereof) that have been simulated. Another potential downside is that the high flexibility of IBMs can quickly lead to models that are *too* complex, and it can be hard to understand what is happening inside the model. That's why, hopefully, our IBMs are simple enough to understand, and provide a gateway into cultural evolution modelling.

## How to use this book - the programming

All of the code in this book is written in [R](https://www.r-project.org). Originally R had a strong focus on statistical data analysis. Its growing user-base has turned R into a more general-purpose programming language. While R is less used for models, it is widely taught in many university departments and is the subject of lots of online tutorials and support forums. It is quite likely that many people have already some experience in R for data analysis and visualisation, so that this experience can be used to approach IBMs, easily than if one needs to learn another programming language. Also, if your IBMs run in R, you can use the same language to analyse the output and plot the results.  

We have used the [bookdown](https://bookdown.org) package to create an html version of the book, which you may well be reading now. This is created from RMarkdown (.Rmd) files, which are a mix of regular text and code. As a reader, you can therefore read the online book and, alongside, run the code using either Rmd file. Of course you can just read the book, but running the code as you go will give you more direct experience of how the code executes, and will allow you to play around with parameters and commands. The best way of learning - especially modelling! - is to try it out yourself.

We assume that the reader has basic knowledge of R (and [RStudio](https://rstudio.com), which provides a powerful user-interface for R), including installing it, setting it up, updating it, installing packages and running code. We strived to proceed from very simple to more complex code in a gradual way, and to explain all the non-obvious newly intorduced programming techniques, but a basic knowledge of R as a programming language, e.g. the use of variables, dataframes, functions, subsetting and loops, will greatly facilitate the reading. 

We use the [tidyverse](https://www.tidyverse.org) package and follow the underlying logic. For example, we use the tidyverse-typical data structures (tibbles rather than dataframes) and the [ggplot](https://ggplot2.tidyverse.org) graphic system (rather than the base R plot function). These are user-friendly and widely used, and they will make it easier to manipulate data and create professional-looking visualisations. The tidyverse, however, has not been thought with IBMs in mind. We have not been thus fanatic and we also used liberally functions, data structures, and programming style that go beyond the tidyverse (in chapter [seven](LINK), for example, we show how matrices are more effective than tibbles in computationally-heavy simulations).  

Beside the tidyverse package, we limited as much as possible the number of additional packages needed to run the simulations. The few packages needed to compile some of the codes are explicitly introduced in the book when needed. 

## How to use this book - the simulations

The book is thought - as the title says - as a step-by-step guide. If you are interested in modelling cultural evolution, or in modelling in general, and you do not have previous experience, you should go through the simulations we describe chapter by chapter. The chapters build in complexity both from the programming and from the conceptual point of view. Alternatively, you may be interested in specific models and you can just go to the chapters that treat them. In this case, however, you will need previous programming experience. (And you will have to figure out by yourself at least some of our programming choices!)

The book is organised as follows. We start presenting individual-based model versions of some of the now-classic analytical and population-level models described in the foundational cultural evolution books, such as Rob Boyd and Pete Richerson's *Culture and the Evolutionary Process* and Luigi-Luca Cavalli-Sforza and Marc Feldman's *Cultural Transmission and Evolution*. The models do not add conceptually to the original analytical treatments, but they show how to use them to develop IBMs, and they provide several basic tools to build models that describe cultural evolution. Some of the chapters develop aspects that are possible only with IBMs, for example simulating cultural dynamics with many different traits (chapter [seven](LINK)).

We then move to what we call 'Advanced topics'. These chapters deals with more recent works in cultural evolution and include different perspectives, or they concerns the usage of analysis that are not customary in cultural evolution modelling (for example network analysis in chapter [eleven](LINK)).

The book does not present any *new* model or perspective on cultural evolution. We are trying to provide an as much as possible up-to-date reflection of the field and, mostly, to show some of the possibilities that IBMs offer to cultural evolutionists. If while reading this book, you have in mind some problem related to cultural evolution and think "I should try to model this!" our goal is reached.

## Conventions and formatting

In general, we followed the [tidyverse style guide](https://style.tidyverse.org) for names of functions, variables, and code formatting.

Name of functions and variables are with underscores to separate words and lowercase letters, e.g. `previous_population`, `biased_mutation`. If in the same chapter we have more than one function for the same model (for example because we add parameters), they are numbered as `unbiased_transmission_1()`, `unbiased_transmission_2()`, etc.

For the text, we used the following conventions:
...
For more on the virtues of formal models for social scientists, see Paul Smaldino's 'Models are stupid, and we need more of them' (2017).

If not, *Hands On Programming With R* by Garrett Grolemund is a good introduction.

R for Data science

B&R 1985

general books on cultual evolution

Other modelling intro