...
For mathematical tractability models often assume well-mixed populations, where any individual is equally likely to interact with any other individual. These models often provide a good approximation to the real-world. Henrich's model from the previous chapter, for example, has shown that a population's ability to maintain and accumulate cultural traits depends on its size, whereby larger populations are more likely to retain and improve more complex cultural traits than smaller ones. This model is a useful tool to study how population-level characteristics (here, demography) play a role in cultural dynamics. Given that individuals acquire information from other individuals through social learning, we might also suspect that individual-level phenomena are important, too. A common tool to represent individual interactions are networks. Networks consist of nodes (also referred to as vertex, _pl._ vertices), which represent individuals, and ties (also referred to as edges) between any two nodes, which indicate that the existence of a relationship (e.g. friendship or kinship, or willingness to cooperate, etc.). Networks can represent relationships between individuals, but just as well between countries, institutions, or any other kind of actors. 
...
```
The above figure shows a simple graph with four nodes (A-D), and their relationship edges (arrows). Not that some nodes have an incoming and an outgoing edge to the another node (e.g. A and C) but some nodes have more incoming than outgoing edges (here A and C). This can be the case when edges indicate relationships that can be unilateral, like friendship or support. In other cases, e.g. kin ship ties, edges are always riciprocal. In this case arrow heads are ommitted and a straight line is drawn between nodes. 

To work with networks in R, we need to find a way to represent nodes and edges. One way to do this is a so called adjacency matrix. An adjcaceny matrix is a square matrix where rows indicate interacting individuals, and columns are individuals who are recipients of this interaction. For example, assume we have $N$ nodes, then our adjacency matrix $A$ will be of the size $N \times N$. In the simplest case we denote the existence of an interaction with a $1$ and the absence of such with a $0$. That is, if $A_{ij}=1$ and $A_{ji}=1$ then we know that there is a reciprocal interaction between node $i$ and $j$. 

Let us translate this into R code. We start be setting up a simple square matrix with a few zeros and ones. Let us also name the rows and columns with capital `Letters`. These will be the names of the nodes. 
...
Sometimes, you might want to create a random network using a symmetric adjacency matrix. A simple way to create a symmetric adjacency matrix is the following: create a random square matrix and then copy the entries from the transposed upper triangle into the entries of the lower triangle:

```{r 11.3}
# Creating a random matrix
random_matrix <- matrix(sample(x = c(0,1), size = 4^2, replace=T), ncol=4)
random_matrix
# It is not symmetric
...
<!-- you should have this function much earlier!--> 
```{r 11.34}
create_network <- function(N, FRIENDSHIPRATIO){
  adjm <- matrix(0, ncol = N, nrow = N)
  adjm[upper.tri(adjm)] <- sample(x = 0:1, 
