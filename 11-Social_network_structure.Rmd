...
```{r 11.16, fig.cap="A cleaner version of the same network."}
...
```{r 11.38, fig.cap="Repeated runs of `gossip_model()` show that on average gossip spreads to less than $0.2$ of the population if individuals have on average 1 friend. For 2 and more friends gossip spreads to more than three quarter of the population."}
data_bnd <- bind_rows(data)
ggplot(data_bnd) +
...
```{r 11.40, fig.cap="Speed at which gossip spreads depending on the average number of friends."}
...
Things to ask Marco:
(i) consider whether set.seed() is really necessary and, if it is, introduce it.
(ii) adjust in the plots of the "Gossip diffusion in networked populations" section the label of the legend and the ticks of x-axis.
