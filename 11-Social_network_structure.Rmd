...
Also note that every time you plot a network, it will look slightly different (the position of nodes changes between plots). To preserve the node positions we will use the `set.seed()` function here. When it is executed before plotting, the nodes will always appear in the same location. You can change the position of the nodes by using a different seed number (here we use 1). The `set.seed()` function affects how your computer generates random numbers. For example, repeatedly running `runif(1)` will draw a different number from a uniform distribution every time. However, if we repeatedly run `set.seed(1); runif(1)` we will always receive the same number. By setting a seed we can recover the same (pseudo) random process between different executions. This is what we are using here when we are plotting the same network repeatedly to retain its original layout. 
...
Note, that we set a seed here. Due to the stochastic nature of this simulation, results might look different between different runs. For illustrative purposes we use a seed here (in this case 5). 
