...
Many simulations assume well-mixed populations, that is, populations of individuals that have an equal chance of encountering each other. In the previous chapter, we have looked at the effects of structured interactions on the transmission of cultural traits. Both, structured and unstructured populations, can be good approximations to the real world, depending on the context and research question. What about structured populations, a combination of the two. That is, a large population of individuals is divided into subsets, where individuals are more likely to encounter individuals from the same subset but much less likely to encounter individuals from a different subset. As such learning would only occur within each subset. However, individuals may migrate between subsets, bringing along their own selection of cultural traits, or might visit another subset and then bring a new cultural trait back home. 
...
We will start very simply with one large population where individuals acquire a behavioural trait from a random individual in their population, and then add layers to the model. Let us set up a population. Because we are only interested which instance of a particular behaviour an individual expresses, each individual can be fully described by its behaviour, and so, we can represent the entire population as a vector of expressed behaviours (`behaviours`). 
...
```{r 12.2}
r_max <- 1000
rec_behav <- tibble(time = 1:t, b1 = 0, b2 = 0)
...
```{r 12.5}
migration_model_1 <- function(n, b, r_max){
  ...
  # replace column names V1, V2, ... with b1, b2, ...
...
# run simulation
res <- migration_model_1(n = 20, b = 2, r_max = 1000)
...
```{r 12.7}
migration_model_2 <- function(n, c, r_max){
  total_pop <- c * n
  ...
  rec_behav_tbl <- as_tibble(rec_behav)
  # replace column names V1, V2, with c1, c2 to represent each cluster
...
# run simulation
res <- migration_model_2(n = 50, c = 4, r_max = 1000)
...
```{r 12.9, fig.cap="When individuals copy behaviours only from individuals within their own subset, we find that the frequency of behaviour 1 becomes uncorrelated between the two subsets. In this example, behaviour 1 is lost in cluster 2, whereas it is still present in cluster 1 at the end of the simulation."}
# turn matrix into tibble
matrix_to_tibble <- function(m){
  m_tbl <- as_tibble(m)
  # replace column names V1, V2, ... with c1, c2, ...
...
migration_model_3 <- function(n, c, r_max){
  total_pop <- c * n
  cluster <- rep(1:c, each = n)
  behaviours <- sample(x = 2, size = total_pop, replace = TRUE)
  rec_behav <- matrix(NA, nrow = r_max, ncol = c)
  ...
  for(round in 1:r_max){
    ...
    rec_behav[round,] <- unlist(lapply(1:c, function(C) sum(behaviours[cluster==C]==1))) / n
  }
  return(matrix_to_tibble(m = rec_behav))
}

res <- migration_model_3(n =20, c=3, r_max = 1000)
ggplot(res) + 
  geom_line(aes(x = time, y = p, col = cluster)) + 
  scale_y_continuous(limits = c(0,1)) + 
  ylab("proportion of behaviour 1 in cluster") + 
...
NOT SURE WE NEED THIS; probably just say it
