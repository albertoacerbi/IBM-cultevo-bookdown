...
for(timestep in 1:t){
  ...
  observer_model <- sample(x = n, size = 2, replace = FALSE)
...
```{r 12.6, fig.cap="The frequency of one out of two behaviours in two sub-populations will fluctuate around 0.5 if individuals are equally likely to learn from individuals in both sub-populations (or clusters)."}
...
```{r 12.7, fig.cap="When individuals copy behaviours only from individuals within their own sub-population, we find that the frequency of behaviour 1 becomes uncorrelated between the two sub-populations. In this example, behaviour 1 is lost in cluster 2, whereas it is still present in cluster 1 at the end of the simulation."}
...
```{r 12.8, fig.cap="When two sub-populations start out fully saturated with one of two different behaviours and if there is no learning between clusters, the intial frequencies will remain unchanged."}
...
```{r 12.10, fig.cap="Without migration between clusters, there is no learning outside a sub-population, and so initial behaviour frequencies remain unchanged."}
...
```{r 12.12, fig.cap="When migration is rare, the frequency of behaviour 1 changes occassionally but quickly bounes back to the original value in the sub-population."}
set.seed(1)
res <- migration_model_5(NPOP = 4, POPSIZE = 100, NBEHAVIOUR = 2, 
...
```{r 12.17, fig.cap="As migration rate increases sub-populations become increasingly similar in the frequency of behaviours.", message=FALSE}
ggplot(repeatedRun) + 
...
In this chapter we have used a simple model to simulate the effect of population sub-structuring. We have seen that, depending on migration (either physical or as migration of ideas) can affect the frequency of a behaviour in each sub-population. When migration is low, the frequency of individual behaviours becomes uncorrelated between sub-populations. However, as migration increases, sub-populations behave more and more like a single large population. 
