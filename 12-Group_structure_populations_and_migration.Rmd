...
Many simulations assume well-mixed populations, that is, populations of individuals that have an equal chance of encountering each other. In the previous chapter, we have looked at the effects of structured interactions on the transmission of cultural traits. Both, structured and unstructured populations, can be good approximations of the real world, depending on the context and research question. What about structured populations, a combination of the two. That is, a large population of individuals is divided into subsets, where individuals are more likely to encounter individuals from the same subset but much less likely to encounter individuals from a different subset. As such learning would only occur within each subset. However, individuals may migrate between subsets, bringing along their own selection of cultural traits, or might visit another subset and then bring a new cultural trait back home. 

## Modelling migration

In this chapter, we will simulate this movement of traits between subsets and study its dynamics. We model a population of size $n$, with $s$ subsets, and $b$ instances of a behavioural trait. For most of this chapter, we assume that there are two versions of this behaviour, for example, two individuals greet each other either with a handshake or by bumping elbows. You will see that there are many opportunities to alter the model (e.g. more subsets, behaviours, learning events, etc.). 

We will start very simply with one large population where individuals acquire a behavioural trait from a random individual in their population, and then add layers to the model. Let us set up a population. Because we are only interested in which instance of a particular behaviour an individual expresses, each individual can be fully described by its behaviour, and so, we can represent the entire population as a vector of expressed behaviours (`behaviours`). 
...
Note that the `table()` function counts each element of a given vector. It is similar to `tabulate()` which we used previously, however, `table()` returns a named vector, so that you know exactly which element is present how many times. 

Each individual in our population expresses only one behaviour. Occasionally, one of the individuals will copy the behaviour of another individual. To simulate this behavioural updating, we a random individual (the observer), which copies a randomly selected behaviour from the population. This approach is identical to the unbiased learning in our earlier chapters. We can simulate repeated updating events by wrapping a `for` loop around the code. Additionally, we will add a record variable (`rec_behav`) to store the frequency of each behaviour.

```{r 12.2, message=FALSE}
library(tidyverse)
r_max <- 1000
...
To plot the results, we first need to turn our "wide" data (where repeated measures/different categories are in the same line) into "long" data (where each line has only one observation/category). While the former is easier to read, the latter is the format that is more useful in combination with `ggplot2`. To "rotate" our data we can use the `pivot_longer()` function. In its arguments we tell it to combine the results of columns `b1` and `b2` into `beaviour`, which indicates the name of the behaviour, and `freq`, which is the frequency of that behaviour. The function will repeat the entries of the `time` column accordingly:

```{r 12.3}
rec_behav_l <- pivot_longer(data = rec_behav, names_to = "behaviour", values_to = "freq", cols=c("b1","b2"))
rec_behav
rec_behav_l
```
...
```{r 12.4, fig.cap="On average the frequenca of two behaviours that are transmitted without a bias will fluctuate around 0.5."}
ggplot(rec_behav_l) + 
...
structured_population_1 <- function(n, b, r_max){
  ...
  rec_behav_tbl <- as_tibble(rec_behav, .name_repair = "minimal")
...
Note that we initially create the matrix `rec_behav`, which we then convert to a `tibble` object. The advantage here is that we can quickly create a matrix of a certain size (here, the number of rounds times number of behaviours), in which we can record the frequencies of each behaviour. The `tibble`, however, will make handling and plotting our data easier. The `.name_repair = "minimal"` argument is for compatibility reasons and should not concern us here. 

Let us test what would happen if we ran this simulation with a very small population size of $n=20$:

```{r 12.6, fig.cap="In smaller populations (here, $N=20$), drift might lead to the sudden exclusion of one of the two behaviours from the group, such that the other behaviour becomes fixed."}
# run simulation
...
We observe that the two behaviours fluctuate around $0.5$ until, by chance, one behaviour is completely replaced by the other one. This is simply due to drift, which affects all small populations or very long time scales.


## Subdivided population with limited contact
Let us move on from this single population example to a version where the population is divided into subsets or `clusters`. For simplicity let us work with only two behaviours here, so that we only need to track the frequency of one behaviour, $p$, in each subset as the frequency of the other is simply $1-p$. We will also assume that there are the same number, $n$, of individuals in each cluster, $c$. 
...
structured_population_2 <- function(n, c, r_max){
  ...
  for(round in 1:r_max){
    ...
    # re_calculate p for each cluster
    for(clu in 1:c){
...
This function is very similar to `migration_omdel_1()` but accounts for the additional clusters. For example, the columns of `rec_behav` now store $p$ for each cluster. We calculate these values in every time step. We could also just calculate it for the cluster of the observing individual, however, if we keep the calculation more general, we will not have to change the code if we allow individuals to permanently move from one cluster to another, in which case we would need to calculate $p$ for both clusters. 
...
```{r 12.8, fig.cap="The frequency of one out of two behaviours in two subsets will fluctuate around 0.5 if individuals are equally likely to learn from individuals in both subsets (or clusters)."}
# run simulation
...
You might have noticed a new function at the very end: `matrix_to_tibble()`. This is a little helper function. As we keep turning matrices into tibbles at the end of our simulation, we can also isolate this process in its own function and re-cycle it in the future versions of our simulation function. Here is the code for the helper function: 

```{r 12.10}
matrix_to_tibble <- function(m){
  m_tbl <- as_tibble(m, .name_repair = "minimal")
...
```{r 12.11, fig.cap="When individuals copy behaviours only from individuals within their own subset, we find that the frequency of behaviour 1 becomes uncorrelated between the two subsets. In this example, behaviour 1 is lost in cluster 2, whereas it is still present in cluster 1 at the end of the simulation."}
...
When we run this simulation repeatedly, you will see that sometimes one of the behaviours gets lost in one, both, or neither of the clusters. Because in this iteration of our simulation there are no interactions between individuals of different clusters, we are essentially simulating three independent populations. 
...
```{r 12.13, fig.cap="Simulation with no contact, $p_c = 0$."}
...
ggplot(res_0) + 
  ...
  theme_bw()
```
```{r 12.14, fig.cap="Simulation with some contact, $p_c = 0.1$."}
ggplot(res_01) + 
  geom_line(aes(x = time, y = p, col = cluster)) + 
  scale_y_continuous(limits = c(0,1)) + 
  ylab("proportion of behaviour 1 in cluster") + 
  theme_bw()
```
```{r 12.15, fig.cap="Simulation with full contact, $p_c = 1$."}
ggplot(res_1) + 
  geom_line(aes(x = time, y = p, col = cluster)) + 
  scale_y_continuous(limits = c(0,1)) + 
  ylab("proportion of behaviour 1 in cluster") + 
...
Let us now simulate migration of individuals (and their cultural trait) between subsets. 

To model (physical) migration, we will add the migration probability $p_m$ to the model (with no migration where $p-m=0$, and always migrating to another subset where $p_m=1$):
...
structured_population_5 <- function(n, c, p_c, p_m, r_max){
  ...
  for(round in 1:r_max){
    ...
    if((runif(1,0,1) <= p_m) & (c > 1)){
      ...
      cluster[ observer_model[1] ] <- sample((1:c)[-clusterid], 1)
    }
...
The migration code chunk is doing two things. First, we make sure that migration only happens with the migration probability $p_m$ (`runif(1,0,1) <= p_m`) and only if the population is actually subset (`c>1`). And second, if the statement is `TRUE`, we choose one new cluster ID among all cluster IDs but without the current one (`sample((1:c)[-clusterid], 1)`). 
...
```{r 12.17, fig.cap="Without migration between clusters, there is no learning outside a subset, and so subsets act as independent populations."}
...
```{r 12.18, fig.cap="When $p_m=1$ the subsets act again as a single population."}
ggplot(res_1) + 
  geom_line(aes(x = time, y = p, col = cluster)) +
  scale_y_continuous(limits = c(0,1)) + 
  ylab("Relative frequency of behaviour 1") + 
...
For $p_m=1$, we find that the frequency of the behaviours become correlated as more and more individuals keep moving between the clusters. Eventually, all subsets will settle on the same behaviour. 

```{r 12.19, fig.cap="When migration is rare, the frequency of behaviour 1 changes occassionally but quickly bounes back to the original value in the subset."}
ggplot(res_01) + 
  geom_line(aes(x = time, y = p, col = cluster)) +
  scale_y_continuous(limits = c(0,1)) + 
  ylab("Relative frequency of behaviour 1") + 
...
We have seen that both the migration and the contact probability affect the distribution of behaviour 1 and 2 in the population. In this last section let us run the model for different pairs of $p_c$ and $p_m$ to more systematically analyse the individual and combined effect of contact and migration probability. To do this, let us slightly change our simulation function and return a measure for how similar (or different) the proportion of behaviour 1 is among the subsets. We could, for example, calculate the variance as a measure for the variability between subsets (using `var(rec_behav)`). In this case, we do not need to store the frequency of behaviour 1 in each subset and for each round (in `rec_behav`), instead we only need to calculate it for the current round:
...
```{r 12.25, fig.cap="Summarised simulation results showing the effect of contact and migration on the distribution of behaviour 1 among subsets of a population."}
ggplot(res_summ) +
  geom_raster(aes(x = factor(p_m), y = factor(p_c), fill = mean_var)) +
  xlab("p_m") + ylab("p_c") + ggtitle("Average variance between subsets") +
  theme_bw() +
